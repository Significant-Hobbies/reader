      ...
      <div className="flex items-start gap-3">
        <div className="min-w-0 flex-1">
          <h3 className="mb-1 line-clamp-1 text-sm font-medium text-white">
            {renderSnippet(result.snippets.find((s) => s.field === 'title')?.text || result.title)}
          </h3>
          ...
          {result.snippets
            ...
            .map((snippet, idx) => (
              <div key={idx} className="mt-1">
                ...
                <p className="line-clamp-2 text-xs text-gray-400">{renderSnippet(snippet.text)}</p>
              </div>
            ))}
...
type LocalArticle = Awaited<ReturnType<typeof getLocalArticles>>[number];

function searchLocalArticles(articles: LocalArticle[], normalizedQuery: string): SearchResult[] {
  return articles
    .filter((article) => {
    ...
    .map<SearchResult>((article) => ({
      ...
      matchedFields: ['local'],
      snippets: [{ field: 'title', text: article.title }],
      relevanceScore: 1,
...
function useSearch(isLocalMode: boolean) {
  ...
  const performSearch = useCallback(
    async (searchQuery: string) => {
      ...
      if (isLocalMode) {
        ...
        setResults(searchLocalArticles(articles, normalizedQuery));
        setIsOpen(true);
        setSelectedIndex(-1);
        return;
      }
...
interface SearchKeyboardConfig {
  isOpen: boolean;
  results: SearchResult[];
  selectedIndex: number;
...
function useSearchKeyboard(config: SearchKeyboardConfig) {
  const { isOpen, results, selectedIndex, inputRef, onResultClick, setSelectedIndex, setIsOpen } =
    config;
  useEffect(() => {
  ...
  }, [isOpen, results, selectedIndex, onResultClick, setSelectedIndex, setIsOpen, inputRef]);
}
