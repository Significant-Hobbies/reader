...
function ChatMessage(props: {
  ...
  const { msg, isLast, isStreaming } = props;
  return (
...
function MessageList(props: {
  ...
  const { messages, isStreaming, chatEndRef } = props;
  return (
...
function ChatInput(props: {
  ...
  const { input, setInput, sendMessage, stopStreaming, isStreaming, page, error } = props;
  return (
...
function PageContext(props: {
  ...
  const { page, messages, importNotice, auth, canImport } = props;
  return (
...
function ChatPanel(props: {
  ...
  const { page, messages, isStreaming, sendMessage, clearChat, chatEndRef } = props;
  return (
    <div className="rounded-xl border border-gray-800 bg-gray-900/60 p-3">
      ...
      {messages.length === 0 ? (
        <div className="space-y-3">
          ...
          {page && (
            <QuickActions onAction={(prompt) => void sendMessage(prompt)} disabled={isStreaming} />
          )}
        </div>
      ) : (
        <MessageList messages={messages} isStreaming={isStreaming} chatEndRef={chatEndRef} />
      )}
    </div>
  );
}

async function doStreamResponse(ctx: {
  config: AIConfig;
  page: PageContent | null;
  ...
  try {
    ...
    while (true) {
      ...
      ctx.setMessages([...ctx.nextMessages, { role: 'assistant', content: fullResponse }]);
    }
    ...
    ctx.setMessages(ctx.nextMessages);
  }
}
